...
stalker is a high-performance SNMP monitoring system designed for large-scale network monitoring. It provides:
...
│                              Clients                                     │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐    │
│  │   stalkerc   │  │   stalkerc   │  │  Custom App │  │     GUI     │    │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘    │
└─────────┼────────────────┼────────────────┼────────────────┼────────────┘
          │                │                │                │
          └────────────────┴───────┬────────┴────────────────┘
...
**Systemd Service:**
```ini
# /etc/systemd/system/stalker.service
[Unit]
...
Type=simple
User=stalker
WorkingDirectory=/opt/stalker
ExecStart=/opt/stalker/bin/stalkerd -config /opt/stalker/config.yaml
Restart=on-failure
RestartSec=5
...
sqlite3 /opt/stalker/data/stalker.db ".backup /backup/stalker-$(date +%Y%m%d).db"

# Or with DuckDB CLI
duckdb /opt/stalker/data/stalker.db -c "EXPORT DATABASE '/backup/export';"
```

**Recovery:**
```bash
# Stop server
systemctl stop stalker

# Restore database
cp /backup/stalker-20240115.db /opt/stalker/data/stalker.db

# Start server
systemctl start stalker
```
...
```bash
# Increase verbosity
stalkerd -config config.yaml -v

# Check specific poller
stalkerc info ns/target/poller
```
...
See `proto/stalker.proto` for complete definitions.
...
```
stalkerc - SNMP Proxy CLI Client

Usage:
  stalkerc [flags] [command]

Flags:
  -server string      Server address (default "localhost:9161")
  -token string       Auth token (or STALKER_TOKEN env)
