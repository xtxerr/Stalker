...
//
// This file handles encrypted secrets storage and retrieval using AES-256-GCM.
package store
...
// Secret represents an encrypted secret without the actual value.
type Secret struct {
	Namespace   string
	Name        string
	SecretType  string
...
func (s *Store) countSecretUsage(namespace, secretName string) (int, error) {
	...
	`, namespace, likePattern).Scan(&nsCount)
	...
	`, namespace, likePattern).Scan(&targetCount)
	...
	s.db.QueryRow(`
		...
		WHERE namespace = ? AND (
			protocol_config LIKE ? ESCAPE '\'
		)
	`, namespace, likePattern).Scan(&pollerCount)
