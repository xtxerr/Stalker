...
build:
	go build -ldflags "-X main.Version=$$(git describe --tags --always --dirty 2>/dev/null || echo dev)" \
		-o bin/stalkerd ./cmd/stalkerd
	go build -o bin/stalkerc ./cmd/stalkerc
	@echo "Built bin/stalkerd and bin/stalkerc"
...
# Development
run:
	go run ./cmd/stalkerd -config config.yaml

cli:
	go run ./cmd/stalkerc -server localhost:9161 -tls-skip-verify
...
release:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build \
		...
		-o bin/stalkerd-linux-amd64 ./cmd/stalkerd
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build \
		-ldflags "-s -w -X main.Version=$$(git describe --tags --always)" \
		-o bin/stalkerd-darwin-amd64 ./cmd/stalkerd
